    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None):
        ...
        if self.two_stage_type == "standard":
            ...
            refpoint_embed_undetach = torch.gather(
                enc_outputs_coord_unselected,
                1,
                topk_proposals.unsqueeze(-1).repeat(1, 1, 20),
            )  # unsigmoid
...
class TransformerDecoder(nn.Module):
    ...
    def forward(
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            ## We want the bbox ie
            x_coords = reference_points_input[..., ::2].reshape(
                reference_points_input.shape[:-1] + (-1,)
            )
            y_coords = reference_points_input[..., 1::2].reshape(
                reference_points_input.shape[:-1] + (-1,)
            )
