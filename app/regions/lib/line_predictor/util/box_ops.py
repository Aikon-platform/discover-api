...
def box_inter(box1, box2):
    ...
    if len(intersection) <= 2:
        return torch.tensor(0, dtype=torch.float, device=box1.device)
...
def cuts(polygons, sizes, p, q):
    ...
    new_polygons = torch.zeros(N, K + 1, 2, device=polygons.device).fill_(
        1e5
    )  # [N, K + 1, 2]
    new_sizes = torch.zeros(N, device=polygons.device).long()  # [N,]
    # print('new_polygons',new_polygons.shape, N, 'new_size',new_sizes[(new_sizes-1)>=5])
    for k in range(K):
...
def box_convex_hull_tensor(boxes1, boxes2):
    ...
    boxes = boxes + 1e-5 * torch.randn([N * M, 8, 2], device=boxes1.device)
    ...
    lower = torch.zeros(N * M, 8, 2, device=boxes.device).fill_(1e10)
    lower_sizes = torch.zeros(N * M, device=boxes.device).long()  # stack top
    upper = torch.zeros(N * M, 8, 2, device=boxes.device).fill_(1e10)
    upper_sizes = torch.zeros(N * M, device=boxes.device).long()  # stack top
    ...
    convex_hull = torch.zeros(N * M, 8, 2, device=boxes.device).fill_(1e10)
    sizes = torch.zeros(N * M, device=boxes.device).long()
