...
class DeformableTransformer(nn.Module):
    def __init__(
        ...
        assert query_dim == 4  # box dim
        self.full_query_dim = query_dim + self.num_parameters
        ...
        if two_stage_type == "standard":
            ...
            if two_stage_learn_wh:
                ...
                self.two_stage_line_wh_embedding = nn.Embedding(1, 2)
                self.two_stage_arc_wh_embedding = nn.Embedding(1, 4)
            else:
    ...
    def init_ref_points(self, use_num_queries):
        self.refpoint_embed = nn.Embedding(use_num_queries, self.full_query_dim)
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None):
        ...
        if self.two_stage_type == "standard":
            if self.two_stage_learn_wh:
                ...
                input_hw_line = self.two_stage_line_wh_embedding.weight[0]
                input_hw_arc = self.two_stage_arc_wh_embedding.weight[0]
            else:
                input_hw = None
                input_hw_line = None
                input_hw_arc = None
            ...
            enc_outputs_bbox_unselected = (
                self.enc_out_bbox_embed(output_memory) + output_proposals
            )
            # (bs, \sum{hw}, self.full_query_dim) unsigmoid
            ...
            refpoint_embed_undetach = torch.gather(
                enc_outputs_bbox_unselected,
                1,
                topk_proposals.unsqueeze(-1).repeat(1, 1, 18),
            )
            refpoint_embed_ = refpoint_embed_undetach.detach()

            init_box_proposal = normalize_parameter(
                torch.gather(
                    output_proposals, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, 18)
                )
            )
            ...
            if self.embed_init_tgt:  # mixed query selection
                tgt_ = (
            ...
            if refpoint_embed is not None:  # for denoising
            ...
            init_box_proposal = normalize_parameter(refpoint_embed_)
        ...
        # Begin Decoder
        #########################################################
        # print("ref without transpose", refpoint_embed.shape)
        # print("ref before decoder", refpoint_embed.transpose(0, 1).shape)
        ...
        if self.two_stage_type == "standard":
            if self.two_stage_keep_all_tokens:
                ...
                ref_enc = enc_outputs_bbox_unselected.unsqueeze(0)
                init_box_proposal = output_proposals

            else:
                hs_enc = tgt_undetach.unsqueeze(0)
                ref_enc = normalize_parameter(refpoint_embed_undetach).unsqueeze(0)
...
class TransformerDecoder(nn.Module):
    def __init__(
        ...
        self.num_feature_levels = num_feature_levels
        self.use_detached_boxes_dec_out = use_detached_boxes_dec_out
        self.full_query_dim = query_dim + num_parameters
        self.ref_point_head = MLP(
            self.full_query_dim // 2 * d_model, d_model, d_model, 2
        )
    ...
    def forward(
        ...
        intermediate = []
        reference_points = normalize_parameter(refpoints_unsigmoid)
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            ratios_list = [valid_ratios] * (self.full_query_dim // 2)
            reference_points_input = (
                reference_points[:, :, None] * torch.cat(ratios_list, -1)[None, :]
            )  # nq, bs, nlevel, 18
            ...
            reference_points_input = reference_points_input[
                ..., 14:18
            ]  # use bounding box as deformable attention reference
            ...
            if self.bbox_embed is not None:
                reference_before_sigmoid = unnormalize_parameter(reference_points)
                delta_unsig = self.bbox_embed[layer_id](output)
                outputs_unsig = delta_unsig + reference_before_sigmoid
                new_reference_points = normalize_parameter(outputs_unsig)
