...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None):
        ...
        if self.two_stage_type =='standard':
            ...
            refpoint_embed_undetach = torch.gather(enc_outputs_coord_unselected, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, 20)) # unsigmoid
    ...
    def forward(self, tgt, memory,
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if self.deformable_decoder:
                if reference_points.shape[-1] == 20:
                    reference_points_input = reference_points[:, :, None] \
                                            * torch.cat([valid_ratios, valid_ratios,valid_ratios,valid_ratios,valid_ratios,valid_ratios, valid_ratios, valid_ratios, valid_ratios, valid_ratios], -1)[None, :] # nq, bs, nlevel, 4
                else:
            ...
            ## last dim of reference_points_input is 8: coordinates of a rectangle
            ## We want the bbox ie 
            x_coords = reference_points_input[..., ::2].reshape(reference_points_input.shape[:-1] + (-1,))
            y_coords = reference_points_input[..., 1::2].reshape(reference_points_input.shape[:-1] + (-1,))
            ...
            if not dropflag:
                output = layer(
                    ...
                    tgt_reference_points = bounding_boxes, #reference_points_input,
