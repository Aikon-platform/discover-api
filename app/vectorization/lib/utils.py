...
def delete_directory(doc_id):
    ...
    try:
        ...
        console(f"An error has occurred when deleting {doc_id} directory", e=e)
        return False
...
def remove_duplicate_circles(circle_coords_list, image_size, circle_scores=None):
    ...
    distances = np.linalg.norm(
        circle_coords[None, :, :] - circle_coords[:, None, :], axis=-1
    )
...
def remove_duplicate_lines(line_coords_list, image_size, line_scores=None):
    ...
    distances = np.minimum(
        np.linalg.norm(line_coords[None, :, :] - line_coords[:, None, :], axis=-1),
        np.linalg.norm(line_coords[None, :, :] - permuted_lines[:, None, :], axis=-1),
    )
    threshold = (image_size[0] + image_size[1]) / 80
    mask = distances < threshold
    indices_to_remove = np.array([np.sum(row[i + 1 :]) for i, row in enumerate(mask)])
...
def remove_duplicate_arcs(line_coords_list, image_size, line_scores=None):
    ...
    permuted_lines = np.hstack(
        (line_coords[:, 2:4], line_coords[:, 0:2], line_coords[:, 4:6])
    )
    distances = np.minimum(
        np.linalg.norm(line_coords[None, :, :] - line_coords[:, None, :], axis=-1),
        np.linalg.norm(line_coords[None, :, :] - permuted_lines[:, None, :], axis=-1),
...
def remove_arcs_on_top_of_circles(
    arc_coords_list, circle_coords_list, image_size, arc_scores=None
):
    ...
    distances = np.linalg.norm(
        circle_coords[None, :, :] - arc_circle_coords[:, None, :], axis=-1
    )
    threshold = (image_size[0] + image_size[1]) / 80
    # print(distances)
    # print(threshold)
...
def remove_arcs_on_top_of_lines(
    arc_coords_list, line_coords_list, image_size, arc_scores=None
):
    arc_coords = np.array(arc_coords_list).reshape(-1, 6)  # (n_arcs, 6)
    line_coords = np.array(line_coords_list).reshape(-1, 4)  # (n_lines, 4)
    line_coords_w_center = np.hstack(
        (
            line_coords[:, :2],
            line_coords[:, 2:],
            (line_coords[:, :2] + line_coords[:, 2:]) / 2,
        )
    )
    line_coords_w_center_permuted = np.hstack(
        (
            line_coords[:, 2:],
            line_coords[:, :2],
            (line_coords[:, :2] + line_coords[:, 2:]) / 2,
    ...
    distances = np.minimum(
        np.linalg.norm(
            line_coords_w_center[None, :, :] - arc_coords[:, None, :], axis=-1
        ),
        np.linalg.norm(
            line_coords_w_center_permuted[None, :, :] - arc_coords[:, None, :], axis=-1
        ),
    )
    threshold = (image_size[0] + image_size[1]) / 50
    mask = distances < threshold
...
def box_xyxy_to_cxcyr(x):
    ...
    """
    x0, y0, x1, y1 = x.T
    b = np.stack([(x0 + x1) / 2, (y0 + y1) / 2, ((x1 - x0) + (y1 - y0)) / 4], axis=-1)
    return b
...
def write_svg_dwg(
    dwg, lines=None, circles=None, arcs=None, show_image=False, image=None
):
    ...
    for circle in circles:
        ...
        if show_image:
            circle_plot = Circle(
                circle[:2], circle[-1], fill=None, color="red", linewidth=1
            )
    ...
    for line in lines:
        ...
        dwg.add(
            dwg.path(
                d="M " + str(p1x) + " " + str(p1y) + " L " + str(p2x) + " " + str(p2y),
                stroke="green",
                stroke_width=3,
                fill="none",
            )
